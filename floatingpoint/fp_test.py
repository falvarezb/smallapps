...
def test_to_exact_decimal_positive():
    bits = str_to_list(
        "0011111111110011001100110011001100110011001100110011001100110011")
    assert to_exact_decimal(bits) == (Decimal(
...
def test_fp_gen():
    ...
    assert next(fp_generator) == (0.00000000000012343, Decimal('0.0000000000001234300000000000102340991445314016317948146994609714965918101370334625244140625'), -43)
    assert next(fp_generator) == (0.00000000000012343000000000004, Decimal('0.00000000000012343000000000003547764811160377940497012878851013084613441606052219867706298828125'), -43)


def test_fp_gen_infinity():
    fp_generator = fp_gen(1.7976931348623157e+308)
    dec_repr, exact_dec_repr, exponent = next(fp_generator)
    assert (dec_repr, exact_dec_repr, exponent) == (1.7976931348623157e+308, Decimal(''), 1023)
    try:
        next(fp_generator)
        assert False
    except OverflowError as e:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("72057594037927945", (17, "72057594037927952")),
